...
export default function EmailModal({ student, handleModalClose }) {
  const dispatch = useDispatch();

  const sendEmail = () => {
    dispatch(
      snackBarMessage(
        `Sent Email to ${student.first_name} ${student.last_name} - (${student.email})`
      )
    );
  };
  ...
  return (
    <>
      <Box sx={style}>
        ...
        <TextField
          label="Subject"
          sx={{ width: "75%", marginTop: 1, padding: 0 }}
        />
        <TextField
          multiline
          sx={{ width: "75%", marginTop: 1 }}
